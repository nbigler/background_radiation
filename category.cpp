...
C_Category::C_Category_set::C_Category_set(unsigned int c) {
	...
	if (tmp.size() > 0)
		category_t_str.push_back(tmp); // Finish off
...
bool C_Category::C_Category_set::get_rules(string & rules_filename) {
	...
	while (infs.good()) {
		...
		if (!infs.good())
			break; // No new line available
		string line = buf;
		if (line.size() == 0)
			break; // Handle empty line
		// Process a rule
		stringstream ss;
		ss << line;
		vector<string> rule;
...
void C_Category::C_Category_set::load_bitmasks() {
	...
	for (size_t i = 0; i < rule_set.size(); i++) {
		...
		for (size_t k = 1; k < rule_set[i].size(); k++) {
			...
			if (bitpos == -1) {
				cerr
						<< "C_Category::C_Category_set::load_bitmasks(): cannot find sign \"";
 ...
 */
bool C_Category::C_Category_set::get_rule_name(size_t rule_num,
		std::string & rule_name) {
	if (rule_num >= rule_set.size())
		return false;
 ...
 */
void C_Category::C_Category_rc_signs::increment(int rc_number,
		unsigned int cset) {
 ...
 *
 */
void C_Category::C_Category_rc_signs::write_csv(ofstream * outfs,
		int rc_number) {
	for (int i = 0; i < enum_count; i++) {
...
void C_Category::increment(C_Category_set combi) {
	...
	if (v == 0)
		v = e_category::Unknown; // Treat empty set as "Unknown"
	ccounters[v]++;
