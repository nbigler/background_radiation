 ...
 *
 *	Compile with:
 *	g++ category.cpp CPersist.cpp main.cpp libs/flowlist.cpp libs/HashMap.cpp libs/lookup3.cpp libs/utils.cpp -lpcap++ -lboost_iostreams -o pcap_writer -Wno-deprecated -O3
 *
 *
...
void usage(char * progname, ostream & outfs)
{
	...
	outfs << "-a			 Count the packets and flows in the files specified with -f\n";
...
void get_flow_count(CPersist &data){
	...
	for (int i=0; i <= data.c.get_rule_count(); i++){
		...
		total_rpackets += packetcount;
	}

	out.close();
	cout << "Total Flows in all rules: " << total_rflows << endl;
	cout << "Total Packets in all rules: " << total_rpackets << endl;
	cout << "--------------" << endl;
