...
Die Sammlung der Daten fand im Zeitraum vom ... bis ... auf einem zwischen dem HSR-Gateway und der Firewall platzierten Server statt, dem ``Kollektor''. Der ``Kollektor ist mit einer leistungsfähigen DAG Endace Netzwerkkarte ausgestattet, die .... Für die Analyse wird ein separater Server, der ``Analyzer'' verwendet auf dem zu Analysezwecken Wireshark installiert wurde.  
\begin{figure}[ht]
	\begin{center}
...
Zusätzlich\marginpar{IDS} zu den Flow- und Signdaten liegen Warnmeldungen des IDS Snort vor, die für die Analyse Anhaltspunkte liefern können. Um die Snort-Alerts mit den Paket-/Flowdaten korrelieren zu können, wurden die IP Adressen in den Pcap-, Flow- und Snort Daten nach dem gleichen Schema anonymisiert.
...
Die Anonymisierung der IP-Adressen ist präfixerhaltend, damit die Subnetzzugehörigkeit erhalten bleibt, lässt aber ansonsten keine Rückschlüsse auf die hinter der Adresse verborgenen Netze, Rechner oder Services zu. Es können also aufgrund der Adresse weder auf die geografische Position der Endknoten, noch auf den Provider Rückschlüsse gezogen werden. Zudem ist es mit der IP allein unmöglich zu bestimmen, ob sich hinter einer bestimmten IP ein regulärer Server befindet. Die IP Adressen der HSR sind statistisch ersichtlich, da bei den Verkehrsdaten mit Ausnahme von Transit- und Multicastverkehr immer eine HSR Adresse involviert ist. Dadurch konnten die Adressen der Form 152.103.xxx.xxx als HSR intern identifiziert werden.
...
Für die Analyse der ICMP Flows nutzen wir die Tatsache, dass Scanning nur mit ICMP Requests möglich ist, da alle anderen ICMP Arten nicht quittiert werden []. Die False Positive Rate ergibt sich bei den ICMP Paketen folglich aus der Anzahl der ICMP Responses.
...
In\marginpar{Charakteris\-tika} die Klasse Suspected Benign fällt der Verkehr, der nicht eindeutig den Klassen ``P2P'' oder ``Service Unreachable'' zugewiesen werden kann, aber keine Anzeichen für bösartiges Verhalten aufweisen. Darunter fallen zum Beispiel ...
