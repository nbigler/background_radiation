...
HashKeyIPv4_3T::HashKeyIPv4_3T(uint32_t * IP, uint8_t * protocol, uint16_t * port){
	*((uint32_t *)(&(key[0]))) = *IP;
...
std::string HashKeyIPv4_3T::printkey() const{
	...
	sprintf(buf,"%u - %s:%u %s:%u (%u)",
			*((uint8_t *)(&(key[4]))),
			tmpip, *((uint16_t *)(&(key[5]))));
	return std::string(buf);
}
...
HashKeyIPv4_4T::HashKeyIPv4_4T(uint32_t * localIP, uint32_t * remoteIP, uint8_t * protocol, uint8_t * direction){
...
std::string HashKeyIPv4_4T::printkey() const{
	...
	sprintf(buf,"%u: %s - %s, %u",
			*((uint8_t *)(&(key[8]))),
			tmpip_src, *((uint16_t *)(&(key[8]))),
			tmpip_dst, *((uint16_t *)(&(key[10]))),
			*((uint8_t *)(&(key[9]))));
	return std::string(buf);
}
...
HashKeyIPv4_5T::HashKeyIPv4_5T(uint32_t * srcIP,
...
HashKeyIPv4_6T::HashKeyIPv4_6T(uint32_t * srcIP,
		uint32_t * dstIP, uint16_t * srcPort,uint16_t * dstPort, uint8_t * protocol, uint8_t * tos){
	*((uint32_t *)(&(key[0]))) = *srcIP;
	*((uint32_t *)(&(key[4]))) = *dstIP;
	*((uint16_t *)(&(key[8]))) = *srcPort;
	*((uint16_t *)(&(key[10]))) = *dstPort;
