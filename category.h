...
enum category_t { category_enum_vals }; // By default enum values are zero-based
	...
	public:
		...
		class C_Category_set {
			...
			public:
				...
				// Add a sign to set.
				void add(e_category::category_t c) { cset |= (1<<c); }

				// Check if a sign is a member of the set.
				bool is_member(e_category::category_t c) { return (cset & (1<<c)); }

				// Show bitset in hexadecimal represenation on console.
				void show() { std::cout << std::hex << "0x" << cset << std::dec << std::endl; }
				...
				void set(unsigned int c) { cset = c; }

				// Get current sign set as a bit set.
				unsigned int get_set() { return cset; }
				...
				int get_enum_count() { return ecnt; }
        ...
        */
		class C_Category_rc_signs {
			...
			public:
				...
				int get_rc_count() { return rc_count; }
		};
