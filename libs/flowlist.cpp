...
/**
  ...
  */
bool CFlowlist::read_flow(boost::iostreams::filtering_istream & infs, struct cflow * cf)
{
	...
	if (num_read == sizeof(struct cflow)) {
		...
		if (cf->magic == 1) {
			bytes += cf->dOctets;
			if (cf->startMs < start_time) start_time = cf->startMs;
			if (cf->startMs > end_time) end_time = cf->startMs;
			return true;
		} else {
...
{
	if (flow_count > 0) {
		if (flow_count > 1) cur_flow = 1; else cur_flow = -1;
		return &(flowlist[0]);
	} else {
		return NULL;
	}
...
{
	if (cur_flow == -1) return NULL;

	struct cflow * p = &(flowlist[cur_flow]);

	if (++cur_flow >= flow_count) cur_flow = -1;
	return p;
}
