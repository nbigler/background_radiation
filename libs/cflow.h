...
//	uint16_t remoteAS;		///< destination AS 
	...
	// Methods
	void init(uint32_t localIP, uint16_t localPort, uint32_t remoteIP, uint16_t remotePort, uint8_t prot, uint8_t flowtype,
				uint64_t	startMs, uint32_t durationMs, uint64_t dOctets, uint32_t dPkts)
	{
...
enum proto_t {UDP, TCP, ICMP, OTHER};
...
enum flow_type_t { outflow=1, inflow=2, uniflow=3, biflow=4, unibiflow=8, allflow=7, q_infl=10, q_outfl= 9, okflow=12,
