...
void reopen_outfile(ofstream & outfs, string ofname) {
	try {
		...
		cerr << "ERROR: re-opening output file " << ofname
				<< " failed.\nTerminating.\n";
		throw;
	}
}
...
void record2String(struct cflow * record, char * out) {
	...
	sprintf(out, "%-4s: %s"
			...
			ipV4ProtocolToString(record->prot).c_str(), dirviz, ip_padding1,
			local, record->localPort, ip_padding2, remote, record->remotePort,
			record->AS.local, record->AS.remote, (unsigned int) record->dOctets,
			record->dPkts, ts.tm_hour, ts.tm_min, ts.tm_sec,
			(long long int) record->startMs % 1000, record->durationMs / 1000,
			record->durationMs % 1000, record->tos_flags, record->magic);
}
...
#define dbg false
	...
	if (dbg) {
		cout << "x=" << x << ", min_fieldsize=" << min_fieldsize
				<< ", numdigits=" << numdigits << ", numsigns=" << numsigns;
		cout << ", numspaces=" << numspaces << ". leadingdigits="
				<< leadingdigits << ", numcommas=" << numcommas << endl;
	}
...
int getSamples(string filename, vector<string> & files) {
	if (filename.size() > 0) {
		...
		try {
			...
			cout << "\n\nERROR: unknown error while opening input file "
					<< filename << "\n\n";
			throw;
		}
		infs.exceptions(ios::goodbit);
...
void print_flow(const cflow & flow) {
	...
	cout << "Flow: " << local << ":" << flow.localPort << ";\t" << remote << ":"
			<< flow.remotePort << ";" << static_cast<int>(flow.dPkts) << endl;
}
...
void print_packet(const packet & pck) {
	...
	cout << "Packet: " << local << ":" << pck.srcPort << ";\t" << remote << ":"
			<< pck.dstPort << ";\t" << static_cast<int>(pck.protocol) << endl;
}
